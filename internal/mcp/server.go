	...
	return []Tool{
		...
		},
		{
			...
			Description: "Guarda o actualiza (upsert si topic_key existe) una firma de memoria sintética estructurada de alta densidad. " +
				"Formato de summary requerido: 'What: <desc> | Why: <motivo> | Where: <archivos> | Learned: <gotchas>'",
			InputSchema: &JSONSchema{
				Type: "object",
				Properties: map[string]Property{
					"title": {
					...
					"summary": {
						Type:        "string",
						Description: "Firma sintética en formato: 'What: ... | Why: ... | Where: ... | Learned: ...'",
					},
					"category": {
						Type:        "string",
				...
				Required: []string{"title", "summary", "category", "tags"},
			},
		},
		{
			Name: "cogni_update",
...
func (s *Server) executeTool(name string, argsRaw json.RawMessage) (string, bool) {
	...
	case "cogni_save":
		...
		if args.Title == "" || args.Summary == "" {
			return "Error: 'title' y 'summary' son requeridos.", true
		}
		if args.Category == "" {
